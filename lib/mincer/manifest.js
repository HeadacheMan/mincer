...
Manifest.prototype.compile = function (files, options) {
  ...
  if ( !Array.isArray(files) ) {
    ...
    destinations = files;
    files = [];
    for (prop in destinations) { files.push(prop); }
  }else{
  ...
  paths.forEach(function (logicalPath) {
    ...
    }else if(destinations[asset.logicalPath] !== undefined) {
      target = path.join(self.dir, destinations[asset.logicalPath]);
    }else{
      target = path.join(self.dir, asset.logicalPath);
    }

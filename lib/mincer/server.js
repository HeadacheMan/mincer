...
Server.prototype.compileBkp = Server.prototype.compile;
Server.prototype.compile = function compile(pathname, bundle, callback) {
  var a;

  a = /([^\/]+)\.(\w{2,3})/;
  if (this.environment.findAsset(pathname, {bundle: !!bundle}) === undefined){
    var matchs = pathname.match(a);
    var newPathname =  pathname.replace(matchs[1]+'/'+matchs[0], matchs[0])
        .replace(matchs[0], matchs[1]+'/'+matchs[0]);
    if (newPathname !== pathname) {
      pathname = newPathname;
    }
  }
  return this.compileBkp(pathname, bundle, callback);
};
